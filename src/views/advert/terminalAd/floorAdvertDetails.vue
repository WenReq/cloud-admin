        ...
        <a-row>
          <a-col class="line-item" :span="12">
            <a-form-item
              ...
              :wrapper-col="{ span: 20 }"
            >
              <a-range-picker v-model="advertDate" class="w100" />
            </a-form-item>
          </a-col>
          <a-col class="line-item" :span="12">
            <a-form-item
            ...
            >
              <div>
                ...
                <div class="upload-tip">
                  <a-icon class="tip-icon" type="info-circle" />上传图片的最佳尺寸：540px*210px，其他尺寸会影响页面效果，格式png，jpeg，jpg。每张大小不超过3M
                </div>
                <a-modal
                  :visible="advertVisible"
                  :footer="null"
                  @cancel="handleAdvertCancel"
                >
  ...
  methods: {
    getAdvInfo (params) {
      getAdvDetails(params).then(res => {
        ...
        this.imageId = res.result.images[0].id
        this.desc = res.result.remark
        this.advertDate = [
          moment(res.result.startTime),
          moment(res.result.endTime)
        ]
    ...
    handleSubmit (e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          const startTime = this.advertDate.length
            ? this.advertDate[0].format('YYYY-MM-DD 00:00:00')
            : ''
          const entTime = this.advertDate.length
            ? this.advertDate[1].format('YYYY-MM-DD 23:59:59')
            : ''
          const params = {}
          ...
          params.endTime = entTime
          const newImgs = []
          newImgs.push({
            path: this.advertFileList[0].response
              ? this.advertFileList[0].response.result
              : this.advertFileList[0].url,
          ...
          if (this.mainTitle === '添加') {
            ...
            params.images[0].id = this.imageId
            this.confirmLoading = true
            editAdv(params)
              .then(res => {
                this.confirmLoading = false
