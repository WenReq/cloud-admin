      ...
      <div class="content">
        <div class="content-l">
          ...
          <a-form-item
            ...
            :wrapper-col="formItemLayout.wrapperCol"
          >
            <div v-if="type === 1 && detail.isWallLogin">
              <a-input :value="detail.isWallLogin" disabled />
            </div>
            <div v-else>
              <a-radio-group v-decorator="['isWallLogin', { initialValue: 0 }]">
                <a-radio :value="0">
                  允许登录
                </a-radio>
                <a-radio :value="1">
        ...
        <div class="content-r border">
          ...
          <div v-if="userType === 2 && type != 1">
            ...
            <a-form-item
              ...
              :wrapper-col="formItemLayout.wrapperCol"
            >
              <div v-if="areaTreeData.length" class="tree rel">
                <a-tree
                  v-model="checkedKeys"
                  checkable
                  default-expand-all
        ...
        <div class="content-r">
          ...
          <div v-if="roleFindMenu.length && type !== 1" class="tree">
            <a-tree
              v-model="checkedMenuKeys"
              :checkable="type !== 1"
              :default-expand-all="true"
              :tree-data="roleFindMenu"
              @check="onCheckMenu"
            />
          </div>
          <div v-if="roleFindMenuViews.length && type === 1" class="tree">
            <a-tree
              :default-expand-all="true"
              :tree-data="roleFindMenuViews"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  data () {
    // this.lastFetchId = 0
    // this.getCardNum = debounce(this.getCardNum, 800)
    return {
      // cityValue: [],
      type: 1,
      userId: '',
  ...
  methods: {
    changeRole (data) {
      if (data && data.length) {
        const params = { roleList: data }
        if (this.type === 2) {
          params.id = this.userId
        }
        this.getRoleFindMenu(params)
      } else {
        // this.roleFindMenu = []
    ...
    getRole (data) {
      const params = { id: this.userId, roleList: data }
      this.getRoleFindMenu(params)
    },
    getRoleFindMenu (params) {
      roleFindMenu(params).then(res => {
        this.roleFindMenu = []
        const result = res.result
        this.checkedMenuKeys = this.handleSelectData(res.result, 'checkedMenu')
        this.selectedData = this.handleDefSelectData(res.result, 'checkedMenu')
        if (result && result.length) {
          result.forEach(item => {
            this.roleFindMenu.push(this.handleTreeData(item))
          })
        }
      })
    },
    ...
    getAreaInfo (list = []) {
      ...
      areaInfo(params).then(res => {
        ...
        this.checkedKeys = this.handleSelectData(result.areaInfo, 'normal')
        this.checkAll = false
        const _area = JSON.parse(JSON.stringify(result.areaInfo))
        if (_area && _area.length) {
          _area.forEach(item => {
    ...
    getDetail () {
      ...
      userDetail(data).then(res => {
        ...
        const defValue = {
          ...
          isWallLogin: result.isWallLogin,
          uPost: result.uPost,
        ...
        const detail = {
          ...
          isWallLogin: result.isWallLogin === 1 ? '禁止登录' : '允许登录',
          userType:
            this.userType === 1
              ? '区域'
              : this.userType === 2
        ...
        // this.cityValue = this.handleRoleInfo(result.provinceInfos)
        this.detail = detail
        ...
        if (this.type === 1) {
          if (result.menus && result.menus.length) {
            result.menus.forEach(item => {
              this.roleFindMenuViews.push(this.handleTreeData(item))
            })
          }
        }
        this.getRole(defValue.roleList)
      })
    },
    changeArea (areasId) {
      this.estateList = []
    ...
    onCheckMenu (checkedKeys, info) {
      // 合并两个数组
      this.confirmCheckedMenuKeys = checkedKeys.concat(info.halfCheckedKeys)
    },
    // 处理默认选中的菜单 UI展示
    handleSelectData (items, flag) {
      const result = []
      function filter (list) {
        for (let i = 0; i < list.length; i++) {
          if (((list[i].children && list[i].children.length === 0) || !list[i].children) && ((list[i].isSelected && flag === 'normal') || (list[i].isSelect && flag === 'checkedMenu'))) {
    ...
    // 处理默认选中的菜单 提交服务器
    handleDefSelectData (items, flag) {
      const result = []
      function filter (list) {
        for (let i = 0; i < list.length; i++) {
          if ((list[i].isSelected && flag === 'normal') || (list[i].isSelect && flag === 'checkedMenu')) result.push(list[i].id)
          if (list[i].children && list[i].children.length) {
            filter(list[i].children)
          }
        }
    ...
    handleRegionData () {
      ...
      if (
        ...
        !this.confirmCheckedKeys.length
      ) {
        // 编辑
        const list = this.handleDefSelectData(this.regionData, 'normal')
    ...
    handleSubmit (e) {
      ...
      validateFields((err, values) => {
        if (!err) {
          ...
          params.menuList = this.confirmCheckedMenuKeys.length === 0 ? this.selectedData : this.confirmCheckedMenuKeys
    ...
    validateUserName (rule, value, callback) {
      ...
      const myreg3 = /^[1][3,4,5,6,7,8,9][0-9]{9}$/
    ...
    handleCheckAll (e) {
      ...
      filter(list)
      this.checkedKeys = selected ? this.handleSelectData(list, 'normal') : []
      this.confirmCheckedKeys = selected ? this.handleDefSelectData(list, 'normal') : []
    }
  }
}
</script>
