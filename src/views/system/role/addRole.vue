...
<script>
import { roleFindMenu, roleInfo, addRole } from '@/api/system'
  ...
  methods: {
    ...
    getRoleMenuList (id) {
      ...
      roleFindMenu(params).then(res => {
        const result = res.result
        this.checkedKeys = this.handleSelectData(result)
        this.type === 2 && (this.menuData = result)
        const _menu = JSON.parse(JSON.stringify(result))
        if (_menu && _menu.length) {
          _menu.forEach(item => {
            this.treeData.push(this.handleTreeData(item))
          })
    ...
    handleSelectData (items) {
      ...
      function filter (list) {
        for (let i = 0; i < list.length; i++) {
          if (list[i].children && !list[i].children.length && list[i].isSelect) {
