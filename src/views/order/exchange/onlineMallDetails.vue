  ...
  methods: {
    getInfo (params) {
      viewOnlineMall(params)
        .then(res => {
          const payType = res.result.payType
          ...
          this.leftTwoInfos.unitPrice = `${res.result.usedCatCoin}${payType === '2' ? '环保值' : '元'}`
          this.leftTwoInfos.exchangeNum = res.result.buyNum
          this.rightTwoInfos.exchangeTime = res.result.exchangeTime
          this.rightTwoInfos.businessName = res.result.businessName
          // this.rightTwoInfos.businessName = '猫先生自营'
          this.rightTwoInfos.giftType = res.result.giftType
          this.rightTwoInfos.usedCatCoin = `${res.result.usedCatCoin}${payType === '2' ? '环保值' : '元'}`
        })
    },
    formatPhone (val) {
