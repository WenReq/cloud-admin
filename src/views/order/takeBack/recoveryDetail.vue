      ...
      <template v-else>
            ...
            <div v-if="order.state == 1 || order.state == 3" class="item">
              ...
              <a-form-item label="接单人员">
                <a-input
                  disabled
                  :value="
                    `${order.receiptUser || ''} ${order.receiptMobile || ''}`
                  "
                />
              </a-form-item>
            </div>
            ...
            <div v-if="order.state == 2" class="item">
              ...
              <a-form-item label="回收总金额">
                <a-input
                  ...
                  :value="
                    `${order.totalPrice}元 ${
                      order.payType ? '现金支付' : '平台支付'
                    }`
                  "
                />
              </a-form-item>
            </div>
            <div v-if="order.state == 2 && order.type != 0" class="item">
              <a-form-item label="服务费">
                <a-input disabled :value="`${order.serviceCharge || 0}元`" />
              </a-form-item>
              <a-form-item label="用户获得金额">
                <a-input
                  disabled
                  :value="
                    `${order.totalPrice - order.serviceCharge}元 （${
                      order.payType ? '现金支付' : '平台支付'
                    }）`
                  "
                />
        ...
        <div class="reserve-info">
          ...
          <div class="list">
            ...
            <div class="item">
              <a-form-item :label="order.type ? '联系人' : '设备名称'">
                <a-input disabled :value="order.name" />
              </a-form-item>
              <a-form-item :label="order.type ? '手机号' : '设备编号'">
                <a-input v-decorator="['mobile']" disabled>
                  <a-button
                    v-if="isShowBtn"
                    slot="suffix"
            ...
            <div class="item">
              <a-form-item :label="order.type ? '上门地址' : '设备地址'">
                <a-input disabled :value="order.areas" />
              </a-form-item>
              <a-form-item label="备注">
                <a-input disabled :value="order.remark" />
...
export default {
  ...
  methods: {
    ...
    handleGoBack () {
      this.$router.push({ path: '/order/takeBack/recovery' })
    }
  }
}
</script>
