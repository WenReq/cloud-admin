        ...
        <a-form ref="searchForm" :form="form">
          ...
          <a-row>
            ...
            <a-col class="search-item" :span="6">
              ...
              >
                <a-input
                  v-decorator="['receiptUser']"
                  allow-clear
                  placeholder="请输入接单人员姓名"
                />
              </a-form-item>
            </a-col>
          </a-row>
    ...
    <div class="card-container mrt16">
      ...
      <div class="card-content">
        ...
        >
          ...
          <template slot="recycleDate" slot-scope="record">
            <span>{{ record.recycleTime }}-{{ record.overTime }}</span>
          </template>
          <template slot="payType" slot-scope="record">
            <span v-if="record.state == 2">{{
              record.payType ? "现金支付" : "平台支付"
            }}</span>
          </template>
          ...
          <span slot="action" slot-scope="record">
            <template>
              ...
              >查看</a-button>
              <a-button
                v-if="record.state == 0 && record.type != 0"
                type="link"
                :size="'small'"
            ...
            >
              ...
              <a-menu slot="overlay">
                <a-menu-item v-if="record.type != 0">
...
const columns = [
  ...
  },
  {
    ...
    dataIndex: '',
    key: 'recycleDate',
    scopedSlots: { customRender: 'recycleDate' }
  },
  {
    title: '超时状态',
    dataIndex: '',
...
export default {
  ...
  data () {
    return {
      ...
      modalform: this.$form.createForm(this, { name: 'modal_form' }),
      cancelForm: this.$form.createForm(this, { name: 'cancel_form' }),
      range (start, end) {
        const result = []
  ...
  methods: {
    ...
    handleTimeList (value) {
      ...
      this.modalform.setFieldsValue({ homeTime: '' })
    },
    dropdownVisibleChange () {
      const date = this.modalform.getFieldValue('homeDate')
      if (!date) {
