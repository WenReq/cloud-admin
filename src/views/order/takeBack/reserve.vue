        ...
        >
          <a-select
            v-decorator="[
              ...
              {
                rules: [
                  { required: true, message: '请输入或选择手机号' },
  ...
  methods: {
    ...
    getPosition (positionInfo) {
      ...
      if (positionInfo.address) {
        ...
        this.orderForm.setFieldsValue({ address: this.handleCommunity() })
      } else {
    ...
    handleTimeList (value) {
      ...
      this.orderForm.setFieldsValue({ homeTime: '' })
    },
    ...
    // 获取字符串的小区
    handleCommunity () {
      const index = this.positionInfo.address.indexOf(
        this.positionInfo.district
      )
      return this.positionInfo.address.substring(
        index + this.positionInfo.district.length
      )
    }
  }
}
