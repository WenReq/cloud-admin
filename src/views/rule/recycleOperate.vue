          ...
          <a-col class="line-item" :span="24">
            ...
            >
              <a-input
                v-decorator="[
                  'deviceTime',
                  { rules: [{ required: true, message: '请填写设备回收清运超时时间' }, {
                    validator: validateToNum,
                  }] }
                ]"
                allow-clear
                addon-after="时" class="commonWidth"
                placeholder="请填写设备回收清运超时时间，只可输入0或者正整数" :disabled="showEdit"
              />
            </a-form-item>
          </a-col>
          <div class="commonTitle mb16">
      ...
      getRule(params).then(res => {
        if (res.result.info) {
          ...
          this.form.setFieldsValue({
            ...
            deviceTime: res.result.info.recyclNewTime,
    ...
    handleSubmit (e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          params.recOverTimeFine = Number(values.penalty)
          params.canOverTimeFine = Number(values.cancelPenalty)
          params.setOverTimeFine = Number(values.allPenalty)
          params.activeReward = Number(values.reward)
          params.timeLimit = Number(values.overtimeLimit)
          params.stopTime = Number(values.residence)
          params.recyclNewTime = Number(values.deviceTime)
          params.prestoreMoney = Number(values.quota)
          params.bail = Number(values.deposit)
