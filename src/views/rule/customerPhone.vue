  ...
  <div class="customerPhone">
    ...
    >
      ...
      >
        ...
        <a-form-model-item
          label="适用场景" prop="handleScenario"
        >
          <a-select
            v-model="form.handleScenario"
            placeholder="请选择适用场景"
            class="w100"
            show-search
            option-filter-prop="children"
        ...
        <a-form-model-item
          label="适用区域" class="areaItem" required prop="area"
        >
          <a-tree-select
            v-model="form.area"
            value=""
        ...
        >
          <a-input
            v-model="form.remark"
            placeholder="请填写备注信息" type="textarea" rows="3" :max-length="50"
          >
          </a-input>
        </a-form-model-item>
      </a-form-model>
    ...
    >
      ...
      <a-form :form="viewForm">
        ...
        <a-form-item label="备注" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }">
          <a-input
            v-model="viewRemark"
            disabled type="textarea" rows="3" :max-length="50"
          >
          </a-input>
        </a-form-item>
      </a-form>
...
export default {
  ...
  data () {
    const validateToCustomPhone = (rule, value, callback) => {
      const myreg = /^[0-9][0-9-]{1,}[0-9]$/
      const phone = /^1(3|4|5|6|7|8|9)\d{9}$/
      if (value && !myreg.test(value) && !phone.test(value)) {
    ...
    return {
      ...
      rules: {
        customPhone: [
          { required: true, message: '请填写客服号码', trigger: 'blur' },
          { validator: validateToCustomPhone, trigger: 'blur' }
        ],
        area: [
          { required: true, message: '请选择客服电话适用的具体区县', trigger: 'change' }
        ]
      },
      areaId: []
    }
  ...
  methods: {
              ...
              if (item.layer === 4) {
                this.editArea.forEach(itemx => {
                  if (item.title === itemx.name) {
                    const vm = this
                    vm.form.area.forEach(function (itemOther, index, arr) {
                      if (itemOther === item.title) {
                        vm.form.area.splice(index, 1, item.value)
                      }
                    })
                    console.log(vm.form.area)
                  }
                })
              }
            })
    ...
    getPhoneInfo (params) {
      viewPhone(params).then(res => {
        if (this.modelTitle === '编辑') {
          ...
          // })
          this.form.customPhone = res.result.info.phone
          this.form.area = this.handleFilterData(res.result.info.areaList, 'name')
          this.areaId = this.handleFilterData(res.result.info.areaList, 'id')
          this.editArea = this.handleFilterAllData(res.result.info.areaList)
          this.form.handleScenario = res.result.info.cusType
          this.form.remark = res.result.info.remark
          ...
          this.viewArea = this.handleFilterData(res.result.info.areaList, 'name').join(',')
    ...
    handleModal (e) {
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          const params = {}
          params.phone = this.form.customPhone
          params.cusType = this.form.handleScenario
          params.remark = this.form.remark
          if (this.modelTitle === '添加') {
            params.menuId = 'b562e02eb2524a9aa4bc599b4423e567'
            params.areas = this.form.area
            ...
            const newArea = JSON.parse(JSON.stringify([...this.form.area]))
