  ...
  <div>
    <div class="card-container">
      ...
      <div class="card-content">
        ...
        <a-row>
          <a-col :span="23">
            <a-tabs default-active-key="1" @change="changeHealth">
              <a-tab-pane key="1" tab="运维记录">
                <a-row>
              ...
              <a-tab-pane key="2" tab="设备健康度">
                <div v-if="healthLoading" align="center">
                  <a-spin size="large" />
                </div>
                <a-empty v-if="!isHealth" />
                <div v-if="isHealth && !healthLoading">
                  <div align="right">
                    数据获取时间：{{ healthObj.createTime }}
                    <a-button icon="sync" style="margin: 0 20px 0 20px;" @click="getHealth()">
                      更新
                    </a-button>
                  </div>
                  <a-row class="health-contain">
                    <a-col :span="21" :offset="2">
                      ...
                      <div class="health-item">
                        ...
                        <div class="health-content">
                          {{ healthObj.hydrogenSulfideConcentration }} mg/m³
                        </div>
                      </div>
                      <div class="health-item">
                        <div class="health-title">
                        ...
                        <div class="health-content">
                          {{ healthObj.ammoniaConcentration }} mg/m³
                        </div>
                      </div>
                      <div class="health-item">
                        <div class="health-title">
                        ...
                        <div class="health-content">
                          {{ healthObj.smokeStatus == 0 ? '正常' : '有烟雾' }}
                        </div>
                      </div>
                    </a-col>
                  </a-row>
        ...
        <a-row>
          <a-col :span="23">
            <a-tabs default-active-key="1" @change="tabChange">
                ...
                <a-row>
                  <a-col v-for="(item, index) in photoRecord.imgList" :key="index" :span="6">
                    <div class="img-container">
                      <div class="table-img">
                        <viewer class="item-img_list" :images="[item.path]">
                          <a v-for="(val, key) in [item.path]" :key="key" href="javascript:;">
                            <img class="td-image" :src="val">
                          </a>
                        </viewer>
              ...
              <a-tab-pane key="0" tab="监控视频">
                <a-list v-if="!ysVideoSrcList.length" class="no-photo" :data-source="[]" />
                <div v-else style="display:flex;justify-content:space-around;flex-wrap:wrap;">
                  <template v-for="(item,index) in ysVideoSrcList">
                    <iframe
                      v-if="item.src"
                      :key="index"
                      class="ysOpenDevice"
                    ...
                    <div v-else :key="index" class="ysOpenDevice">
                      {{ item.name + '(' + item.msg + ')' }}
                    </div>
                  </template>
...
export default {
  ...
  methods: {
    ...
    getHealth () {
      ...
      this.$httpPost(this.$basePath.getRecords, { type: '8', productId: this.$route.query.id })
        .then(res => {
          this.healthLoading = false
          if (res.result) {
            res.result.createTime = moment(res.result.createTime).format('YYYY-MM-DD HH:mm:ss')
            this.healthObj = res.result
          } else {
            this.isHealth = false
          }
        })
    ...
    tabChange (type) {
      ...
      this.type = type
      if (type === '0') {
        this.$httpPost(this.$basePath.getEzopenByProductId, {
          ...
          .then(res => {
            ...
            res.result.forEach(item => {
              if (item.state === 0) {
                this.ysVideoSrcList.push({
                  src: `https://open.ys7.com/ezopen/h5/iframe?url=${item.url}&autoplay=1&accessToken=${item.token}`
                })
              } else {
                this.ysVideoSrcList.push({
                  msg: item.msg,
