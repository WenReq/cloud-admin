...
export default {
  ...
  mounted () {
    if (this.$route.name === 'viewCar') {
      ...
      this.isEdit = false
      this.getPersonList(this.userType)
    }
  ...
  methods: {
    getDetail () {
      this.$httpPost(this.$basePath.getCarInfo, {
        ...
        .then(res => {
          ...
          this.getPersonList(data.userType)
    ...
    getPersonList (userType) {
      this.$httpPost(this.$basePath.getRecycleUserList, { userType: userType || 0 })
        .then(res => {
    ...
    changeType (type) {
      ...
      this.form.setFieldsValue({ recycleUserId: '' })
    },
